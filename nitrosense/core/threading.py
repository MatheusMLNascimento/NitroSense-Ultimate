...
1. All signals use Qt.ConnectionType.UniqueConnection (no double-connects)
2. Explicit cleanup of connections on thread stop
3. Only @pyqtSlot() methods receive signals (type safety)
4. No direct widget access from threads—use signals only
5. Proper quit()/wait() protocol for clean thread shutdown
"""

import time
from typing import Dict, Any, Optional, Callable
from PyQt6.QtCore import QThread, QThreadPool, QRunnable, pyqtSignal, QObject, Qt
...
class HardwareSignals(QObject):
    """Signals emitted by hardware worker—thread-safe."""
...
class HardwareWorker(QThread):
    ...
    THREAD SAFETY:
    - Uses Qt signals exclusively for IPC
    - Signals are connected with Qt.ConnectionType.UniqueConnection
    - Proper quit/wait on cleanup
    """
    ...
    def __init__(self, hardware_manager, update_interval: int = 1500):
        ...
        logger.info("✓ HardwareWorker initialized (thread-safe)")

    def request_stop(self):
        """Request graceful stop."""
        self._stop_requested = True
        logger.info("HardwareWorker stop requested")

    def run(self):
        """Main worker loop with clean shutdown and error handling."""
        ...
        logger.info("🧵 HardwareWorker exited cleanly")

    def stop(self):
        """Stop worker thread gracefully with timeout."""
        logger.info("Requesting HardwareWorker to stop...")
        self.request_stop()
        
        # Wait for thread to finish with timeout
        if not self.wait(5000):  # 5 second timeout
            logger.warning("HardwareWorker did not stop gracefully—forcing quit")
            self.quit()
            self.wait(2000)
