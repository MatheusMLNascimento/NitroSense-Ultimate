...
class StatusPage(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        title = QLabel("System Status Monitoring")
        title.setFont(QFont("Segoe UI", 18, QFont.Weight.Bold))
        title.setStyleSheet(f"color: {COLOR_SCHEME['primary']};")
        layout.addWidget(title)

        subtitle = QLabel("CPU, GPU and platform health in one glance.")
        ...
        for name, key, unit in metrics:
            frame = QFrame()
            frame.setStyleSheet(
                f"background-color: {COLOR_SCHEME['surface']}; border: 1px solid #3a3a3a; border-radius: 12px; padding: 10px;"
            )
            ...
            metric_label.setStyleSheet(f"color: {COLOR_SCHEME['text_secondary']};")
            frame_layout.addWidget(metric_label)
    ...
    def _update_status(self) -> None:
        ...
        try:
            ...
            gpu_ok = gpu_usage is None or gpu_usage < 98
            gpu_status = f"GPU Load: {gpu_usage:.0f}%" if gpu_usage is not None else "GPU Load: N/A"
            gpu_detail = (
                f"Temp: {gpu_temp:.0f}°C • {gpu_mem_used}/{gpu_mem_total}MB"
                if gpu_usage is not None and gpu_mem_used is not None and gpu_mem_total is not None
                else (f"Temp: {gpu_temp:.0f}°C" if gpu_temp else "Temp: N/A")
            )
            self._update_block("gpu", gpu_ok, gpu_status, gpu_detail)

            self._update_block("nbfc", self.hardware.nbfc_available, "NBFC Active", "Service responsive")
    ...
    def _get_gpu_stats(self):
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                gpu_usage, mem_used, mem_total = [item.strip() for item in result.stdout.split(",")]
                return float(gpu_usage), int(mem_used), int(mem_total)
        except Exception as exc:
